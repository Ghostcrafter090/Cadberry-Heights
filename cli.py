...
def register_start_arguments(parser: ArgumentParser):
    ...
    parser.add_argument("-n", "--password", help=_("args.start.login"), metavar="passVar")
...
def cmd_start(ns: Namespace, ctx: CliContext):
    ...
    try:
        ...
        if ns.login is not None:
            start_opts.auth_session = prompt_authenticate(ctx, ns.login, not ns.temp_login, ns.microsoft, ns.password, ns.anonymise)
...
def cmd_login(ns: Namespace, ctx: CliContext):
    sess = prompt_authenticate(ctx, ns.email_or_username, True, ns.microsoft, ns.passVar)
...
def prompt_authenticate(ctx: CliContext, email: str, cache_in_db: bool, microsoft: bool, passVar: str, anonymise: bool = False) -> Optional[AuthSession]:
    ...
    try:
        if microsoft:
            ...
            session = prompt_yggdrasil_authenticate(auth_db.get_client_id(), email, passVar)
        if session is None:
            return None
        if cache_in_db:
...
def prompt_yggdrasil_authenticate(client_id: str, email_or_username: str, passVar: str) -> Optional[YggdrasilAuthSession]:
